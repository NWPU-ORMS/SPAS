...
class PRAU2_SV:
    ...
    def CreateModel(self):
        ...
        for p in range(1, self.instance.patients+1):
            ...
            if flag_alpha2 == 1:
                alpha2_index.append(p)
        self.alpha = self.model.addVars(alpha_index, vtype=GRB.BINARY, name=['alpha_'+str(p)+'_'+str(d) for p, d in alpha_index])
        self.alpha2 = self.model.addVars(alpha2_index, vtype=GRB.BINARY, name=['alpha2_'+str(p) for p in alpha2_index])
        ...
        t0_index = []
        for p in range(1, self.instance.patients+1):
            if self.instance.L_p[p] >= 1 and self.instance.patient_state[p]==1:
                t0_index.append((p))
        self.t0 = self.model.addVars(t0_index, vtype=GRB.BINARY, name=['t0_'+str(p) for p in t0_index])
        ...
        for p in range(1, self.instance.patients+1):
            if self.instance.patient_state[p] == 0:
                for d in range(self.instance.planning_horizon):
                    if d >= self.instance.DA_p[p][0] and d <= self.instance.DA_p[p][1]:
                        obj += self.instance.W_DE * (d - self.instance.DA_p[p][0]+ self.instance.delay[p]) * self.alpha[p, d]
            
        for p in alpha2_index:
            if self.instance.patient_state[p] == 0:
                obj += self.instance.W_DE1 * (self.instance.planning_horizon - self.instance.DA_p[p][0] + self.instance.delay[p]) * self.alpha2[p]
        ...
        for p in range(1, self.instance.patients+1):
            if self.instance.L_p[p] >= 1 and self.instance.patient_state[p]==1:
                obj += self.instance.W_tr * self.t0[p]

        # 与期望场景相关的费用
        for p in range(1, self.instance.patients+1):
            if self.instance.patient_state[p] == 0 or self.instance.patient_state[p] == 1:
                for r in range(self.instance.rooms):
                    if self.instance.R_p[p][r] == 1:
                        for i in range(self.instance.planning_horizon):
                            if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                                for d in range(self.instance.planning_horizon):
                                    if d >= i + self.instance.L_p[p] and d <= i + self.instance.DU_p[p]['max_stay'] - 1:
                                        ...
                                        # print(p, r, i, dd1)
            elif self.instance.patient_state[p] == 2:
                for i in range(self.instance.planning_horizon):
                    if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                        for d in range(self.instance.planning_horizon):
                            if d >= i + self.instance.L_p[p] and d <= i + self.instance.DU_p[p]['max_stay'] - 1:
                                r = self.instance.patient_room_last_day[p]
                                obj += self.instance.C_pr[p][r] * self.instance.DU_p[p][d - i + 1]
        ...
        # 约束1-1
        for p in range(1, self.instance.patients+1):
            if self.instance.patient_state[p] == 1 or self.instance.patient_state[p] == 2:
                self.model.addConstr(self.alpha[p, 0]== 1, name='C1-1_'+str(p))
            ...
            if self.instance.patient_state[p] == 1:
                for i in range(self.instance.planning_horizon):
                    if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                        ...
                        linexp += self.t0[p]
                        r = self.instance.patient_room_last_day[p]
                        self.model.addConstr(linexp >= 1- self.x[p, r, i, d], name='C6-1_'+str(p)+'_'+str(d))
                                ...
                                for i in range(self.instance.planning_horizon):
                                    if i >= self.instance.DA_p[pp1][0] and i <= self.instance.DA_p[pp1][1] and d >= i  and d <= self.instance.DU_p[pp1]['max_stay'] + i -1 :
                                        ...
                                        if d < i + self.instance.L_p[pp1]:
                                            ...
                                            elif self.instance.patient_state[pp1] == 2 and r == self.instance.patient_room_last_day[pp1]:
                                                linexp -= 1
                                    ...
                                    if i >= self.instance.DA_p[pp1][0] and i <= self.instance.DA_p[pp1][1] and d >= i  and d <= self.instance.DU_p[pp1]['max_stay'] + i -1 :
                                        ...
                                        if self.instance.patient_state[pp1] == 0 or self.instance.patient_state[pp1] == 1:
                                            if d < i + self.instance.L_p[pp1]:
                                                ...
                                                dd1 = i + self.instance.L_p[pp1] - 1
                                                linexp -= self.x[pp1, r, i, dd1]
                                        if self.instance.patient_state[pp1] == 2 and r == self.instance.patient_room_last_day[pp1]:
                                            linexp -= 1
    ...
    def Solve(self, time_limit = 0, log_file = '', input_solution = None, deadline = None):
        ...
        if input_solution != None:
            for p in range(1, self.instance.patients+1):
                # print("患者",p,"状态：", self.instance.patient_state[p])
                ...
                if Ad==-1:
                    self.alpha2[p].Start = 1
                    # self.alpha[p, Ad].lb = 1
                else:
                    for d in range(self.instance.planning_horizon):
                        if d >= self.instance.DA_p[p][0] and d <= self.instance.DA_p[p][1]:
                            if d == Ad:
                        ...
                        if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                            for d in range(self.instance.planning_horizon):
                                if d >= i and d < i + self.instance.L_p[p]:
                                    r1 = -1 #初始化r1 取值
                                    if i1 == i:
                                        r1 = input_solution[p][d]
                                    for r in range(self.instance.rooms):
                                        if self.instance.R_p[p][r] == 1:
                                            if r == r1 and i1 == i:
                                                self.x[p, r, i, d].Start = 1
                                            else:
                                                self.x[p, r, i, d].Start = 0
        ...
        if self.model.getAttr('Status') != 3 and self.model.getAttr('SolCount') > 0:
            for p in range(1, self.instance.patients+1):
                ...
                if flag_alpha2 == 0:
                    ...
                    solution[p].update({'Admission':-1}) #本轮未能安排
        
            solution.update({'Reject_count':total_reject_count})
