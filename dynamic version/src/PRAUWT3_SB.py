...
class PRAUWT_SB:
    ...
    def EliteScenarioCreate2(self):
        ...
        for o in range(self.modified_scenario):
            ...
            for p in range(1, self.instance.patients+1):
                l = self.instance.L_p[p]
                ms = self.instance.DU_p[p]['max_stay']
                for j in range(1, ms-l+1):
                    if ratio <= self.instance.DU_p[p][l + j]:
                        dd1 = j
                    else:
                        dd1 = j - 1
    ...
    def CreateModel(self):
            ...
            if self.instance.patient_state[p] == 0: 
                for r in range(self.instance.rooms):
                    if self.instance.R_p[p][r] == 1:
                        for i in range(self.instance.planning_horizon):
                            if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                                obj += self.instance.W_DE * (i - self.instance.DA_p[p][0]+ self.instance.delay[p]) * self.x[p, r, i]
                
                obj += self.instance.W_DE1 * (self.instance.planning_horizon - self.instance.DA_p[p][0]+ self.instance.delay[p])
                for r in range(self.instance.rooms):
                    if self.instance.R_p[p][r] == 1:
                        for i in range(self.instance.planning_horizon):
                            if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                                obj -= self.instance.W_DE1 * (self.instance.planning_horizon - self.instance.DA_p[p][0]+ self.instance.delay[p]) * self.x[p, r, i]
                ...
                if self.instance.patient_state[p] == 0 or self.instance.patient_state[p] == 1:
                    ...
                    for i in range(self.instance.planning_horizon):
                        if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                            for d in range(self.instance.planning_horizon):
                                if d >= i + self.instance.L_p[p] and d < i + self.instance.L_p[p] + self.modified_L_pu[p][w]:
                                    obj += self.instance.C_pr[p][r] * 1/self.modified_scenario
                        ...
                        for i in range(self.instance.planning_horizon):
                            if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                                if d >= i and d < i + self.instance.L_p[p]:
                                    ...
                                    num += 1
                if num > 0:
                    self.model.addConstr(linexp >= 0, name='C2-1_'+str(r)+'_'+str(d))
        ...
        for p in range(1, self.instance.patients+1):
            if self.instance.patient_state[p] == 1:
                ...
                for i in range(self.instance.planning_horizon):
                    if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                        self.model.addConstr(self.t0[p] >= 1 - self.x[p, r, i], name='C1-2_'+str(p)+'_'+str(r)+'_'+str(i))
                ...
                for w in range(self.modified_scenario):
                    ...
                    for p in range(1, self.instance.patients+1):
                        if self.instance.patient_state[p] == 0 or self.instance.patient_state[p] == 1:
                            ...
                            if r == self.instance.patient_room_last_day[p]:
                                for i in range(self.instance.planning_horizon):
                                    if i >= self.instance.DA_p[p][0] and i <= self.instance.DA_p[p][1]:
                                        if d >= i and d < i + self.instance.L_p[p] + self.modified_L_pu[p][w]:
                                            linexp -= 1
    ...
    def Solve(self, time_limit = 0, log_file = '', input_solution = None, deadline = None):
        ...
        self.model.setParam('MIPGAP', 0.01) # 设置MIPGAP为1%
        if input_solution is not None:
            for (p, r, i), variable in self.x.items():
                admission = input_solution[p]['Admission']
                selected = admission == i and input_solution[p].get(i) == r
                variable.Start = int(selected)
            for p, variable in self.t0.items():
                r0 = input_solution[p].get(0)
                r1 = self.instance.patient_room_last_day[p]
                variable.Start = int(r0 is not None and r0 != r1)
        if deadline is not None:
